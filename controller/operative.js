...
exports.addReportWithInfo = async (req, res) => {  
  const {doctorName,campDate,campCenter,address,zone,city,state,hospitalName,userId,subCatId,brandId,brandReplaced} = req.body;
  console.log(req.body);
  const formattedCampDate = moment(campDate, 'DD-MM-YYYY').format('YYYY-MM-DD');
  const query = 'insert into operative_mst (doctor_name,camp_date,center_name,address,state_id,city_id,zone_id,hospital_name,brand_replaced,brand_name,subcat_id,created_by,modified_by) values(?,?,?,?,?,?,?,?,?,?,?,?,?)';
  
  
  try {
    db.query(query, [doctorName,formattedCampDate,campCenter,address,state,city,zone,hospitalName, brandReplaced,brandId,subCatId,userId,userId], (err, result) => {
      if (err) {
        logger.error(`Error in /controller/operative/addReportWithInfo: ${err.message}. SQL query: ${query}`);
        res.status(500).json({
          errorCode: "0",
          errorDetail: err,
          responseData: {},
        ...
        const ocrid = result.insertId;
        logger.info('Post- Operative report added Successfully');

        res.status(200).json({ message: "Post- Operative Report Added Successfully",errorCode: "1",ocrid})
      } 
    });
  } catch (error) {
    logger.error(`Error in /controller/operative/addReportWithInfo: ${error.message}`);
    res.send(error)
  }
};
...
exports.updateReportWithInfo = async (req,res)=>{
  ...
  const {doctorName,campDate,campCenter,address,zone,city,state,hospitalName,userId,brandId,orId,brandReplaced} = req.body;
  
  const formattedCampDate = moment(campDate, 'DD-MM-YYYY').format('YYYY-MM-DD');
   const query = 'CALL UpdateOperativeCampReportWithInfo(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
  try {
    db.query(query, [orId, doctorName,formattedCampDate,campCenter,address,zone,city,state,hospitalName,brandReplaced,userId,brandId], (err, result) => {
      if (err) {
        console.log(err)
        logger.error(`Error in /controller/operative/updateReportWithInfo: ${err.message}. SQL query: ${query}`);
        res.status(500).json({
    ...
    logger.error(`Error in /controller/operative/updateReportWithInfo: ${error.message}.`);
    res.send(error)
  }
}
...
exports.getReportInfoWithId = async (req, res) => {
  const {orId} = req.body;
  const query = `select operative_mst.doctor_name,operative_mst.camp_date,
  operative_mst.hospital_name,operative_mst.brand_replaced,operative_mst.brand_name,
  operative_mst.subcat_id,operative_mst.created_by,operative_mst.center_name,
  operative_mst.address,operative_mst.state_id, operative_mst.city_id,operative_mst.zone_id,
  ...
  where operative_mst.orid = ? and operative_mst.status ='Y'`
  
  try {
    db.query(query,[orId],(err, result) => {
      if (err) {
        logger.error(`Error in /controller/operative/getReportInfoWithId: ${err.message}. SQL query: ${query}`);
    ...
    logger.error(`Error in /controller/operative/getReportInfoWithId: ${error.message}.`);
    res.send(error)
  }
};  
...
exports.getAllCampReport = async (req, res) => {
  ...
  const query = 'select orid,doctor_name,camp_date from operative_mst where created_by = ? and subcat_id = ? and status = "Y"'

  //const query = 'CALL GetAllCampReport(?,?)'
  try {
    db.query(query,[userId,subCatId],(err, result) => {
      if (err) {
        logger.error(`Error in /controller/operative/getAllCampReport: ${err.message}. SQL query: ${query}`);
    ...
    logger.error(`Error in /controller/operative/getAllCampReport: ${error.message}.`);
    res.send(error)
  }
};
...
exports.getBrandWithId = async (req, res) => {
 ...
 const query = 'select basic_id,description from basic_mst where code =?'
...
exports.deleteReportWithId = async (req, res) => {
  ...
  try {
      const query1 = "UPDATE operative_mst SET status = 'N' WHERE orid = ?";
      ...
      if (result1.affectedRows === 0 || result3.affectedRows === 0) {
    ...
    logger.error(`Error in /controller/operative/deleteReportWithId: ${err.message}.`);
