...
exports.addReportWithInfo = async (req, res) => {  
  const {campDate,mrcode,campCenter,address,zone,city,state,doc1,doc2,doc3,doc4,userId,subCatId} = req.body;
  const formattedCampDate = moment(campDate, 'DD-MM-YYYY').format('YYYY-MM-DD');
  const query = 'insert into glaucoma_report_mst (camp_date,center_name,address,zone_id,city_id,state_id,mr_code,doc_name1,doc_name2,doc_name3,doc_name4,subcat_id,created_by,modified_by) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
  
  //const query = 'CALL AddCampReportWithInfo(?, ?, ?, ?, ?)'
  try {
    db.query(query, [formattedCampDate,campCenter,address,zone,city,state,mrcode,doc1,doc2,doc3,doc4,subCatId,userId,userId], (err, result) => {
      if (err) {
        logger.error(`Error in /controller/glaucomareport/addReportWithInfo: ${err.message}. SQL query: ${query}`);
        res.status(500).json({
          errorCode: "0",
        ...
        const gcrid = result.insertId;
        logger.info('Glaucoma Camp Report added Successfully');

        res.status(200).json({ message: "Glaucoma Camp Report Added Successfully",errorCode: "1",gcrid})
      } 
    });
  } catch (error) {
    logger.error(`Error in /controller/glaucomareport/addReportWithInfo: ${error.message}`);
    res.send(error)
  }
};
...
exports.updateReportWithInfo = async (req,res)=>{
  ...
  const {campDate,campCenter,address,zone,city,state,mrcode,doc1,doc2,doc3,doc4,userId,grId} = req.body;
  
  const formattedCampDate = moment(campDate, 'DD-MM-YYYY').format('YYYY-MM-DD');
   const query = 'CALL UpdateGlaucomaCampReportWithInfo(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
  try {
    db.query(query, [grId,formattedCampDate,campCenter,address,zone,city,state,mrcode,doc1,doc2,doc3,doc4,userId], (err, result) => {
      if (err) {
...
exports.getReportInfoWithId = async (req, res) => {
  const {grId} = req.body;
  const query = `select glaucoma_report_mst.camp_date,glaucoma_report_mst.mr_code,
  glaucoma_report_mst.doc_name1,glaucoma_report_mst.doc_name2,glaucoma_report_mst.doc_name3,
  glaucoma_report_mst.doc_name4, glaucoma_report_mst.center_name,glaucoma_report_mst.address,
  glaucoma_report_mst.state_id,glaucoma_report_mst.city_id,glaucoma_report_mst.zone_id,
  glaucoma_report_mst.subcat_id,glaucoma_report_mst.created_by, user_mst1.name,
  ...
  LEFT JOIN user_mst1 ON glaucoma_report_mst.mr_code = user_mst1.empcode
  where glaucoma_report_mst.grid = ? and glaucoma_report_mst.status ='Y'`
  
  try {
    db.query(query,[grId],(err, result) => {
      if (err) {
        logger.error(`Error in /controller/glaucomareport/getReportInfoWithId: ${err.message}. SQL query: ${query}`);
    ...
    logger.error(`Error in /controller/glaucomareport/getReportInfoWithId: ${error.message}.`);
    res.send(error)
  }
};  
...
exports.getAllCampReport = async (req, res) => {
  ...
  const query = `select glaucoma_report_mst.grid,glaucoma_report_mst.doc_name1, 
  glaucoma_report_mst.doc_name2, glaucoma_report_mst.doc_name3,
  glaucoma_report_mst.doc_name4, 
  glaucoma_report_mst.camp_date,user_mst1.name
  from 
  glaucoma_report_mst 
  LEFT JOIN user_mst1 ON glaucoma_report_mst.mr_code = user_mst1.empcode
  where 
  glaucoma_report_mst.created_by = ? and glaucoma_report_mst.subcat_id = ?
  ...
  try {
    db.query(query,[userId,subCatId],(err, result) => {
      if (err) {
        logger.error(`Error in /controller/glaucomareport/getAllCampReport: ${err.message}. SQL query: ${query}`);
    ...
    logger.error(`Error in /controller/glaucomareport/getAllCampReport: ${error.message}.`);
    res.send(error)
  }
};
...
exports.deleteReportWithId = async (req, res) => {
  ...
  try {
      const query1 = "UPDATE glaucoma_report_mst SET status = 'N' WHERE grid = ?";
